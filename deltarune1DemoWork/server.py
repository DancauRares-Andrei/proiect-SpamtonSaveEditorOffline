...
ROOT_DIRECTORY = r"/home/rares/Descărcări/SpamtonSaveFIleEditor/Demo_Chapter1"
PORT = 8080
...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/deltarune1Demo/savefile/update":
            content_length = int(self.headers['Content-Length'])
            post_data = self.rfile.read(content_length).decode('utf-8')
            form_data = urllib.parse.parse_qs(post_data)

            input_file = os.path.join(ROOT_DIRECTORY, "filech1_0_demo")
            if not os.path.exists(input_file):
                self.send_error(500, "Fișierul original filech1_0_demo nu există.")
                return 
            ...
            # Scrie fișierul
            file_path = "./filech1_0"  # Scriem într-un path temporar
